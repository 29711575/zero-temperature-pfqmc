...
class  SpinlessTvChainUtils : public SpinlessTvUtils {
  public:
    ...
    inline void KineticGenerator(MatType &H) const {
        H.setZero();
        int idx1, idx2;
        DataType tmp = (std::complex<double>(0.0, 1.0));
        DataType tmpDelta = (std::complex<double>(0.0, 1.0))*delta;
        DataType tmpMu = (std::complex<double>(0.0, 1.0))*mu;
        // int Li = Lx;
        // if (boundaryType == 1) {
        //     Li = Lx - 1;
        // }
    ...
    inline DataType EdgeCorrelator(const MatType &g) {
        ...
        return (std::complex<double>(0.0, 1.0)) * g(idx1, idx2);
    }
    ...
    inline DataType StructureFactorCDWOffset(const MatType &g) const {
        ...
        for (int i = 0; i < Lx; i++) {
            for (int j = 0; j < Lx; j++) {
                ...
                tmp = (-g(idxi1, idxj1) * g(idxi2, idxj2) + g(idxi1, idxi2) * g(idxj1, idxj2) + g(idxi1, idxj2) * g(idxi2, idxj1));
                if ( (i+j) % 2 == 0) {
                    r += tmp * exp(std::complex<double>(0.0, 1.0) * deltaq * double(i - j) );
                } else {
                    r -= tmp * exp(std::complex<double>(0.0, 1.0) * deltaq * double(i - j) );
                }
            }
        ...
        for (int i = 0; i < Lx; i++) {
            for (int j = 0; j < Lx; j++) {
                ...
                for (int k=0; k<Lx; k++) {
                    ...
                    r(k) += tmp * exp(std::complex<double>(0.0, 1.0) * curQ * double(i - j) );
                }
            }
        }
        out = r / (4.0 * Lx * Lx);
    ...
    inline DataType Z2FermionParity(const MatType &g) const {
        ...
        switch (Lx % 4) {
            case 0:
                ...
                break;
            case 1:
                additionalSign = -(std::complex<double>(0.0, 1.0));
                break;
            case 2:
                additionalSign = -1.0;
                break;
    ...
    inline DataType Z2FermionParityEdgeCorrelator(const MatType &g) const {
        ...
        return (std::complex<double>(0.0, 1.0)) * additionalSign * pfaf(Lx-1, gcopy);
    }

    inline DataType energyFromGreensFunc(const MatType & g) {
        DataType r = 0.0;
        DataType tmp = ((0.5 * std::complex<double>(0.0, 1.0)));
        DataType tmpDelta = ((0.5 * std::complex<double>(0.0, 1.0))) * delta;
        DataType tmpMu = ((0.5 * std::complex<double>(0.0, 1.0))) * mu;
        int idx1, idx2;
        int Li = Lx;
