...
public:
    ...
    // Optional two-phase auxiliary-field proposal interface.  The base
    // implementation keeps all legacy operators source-compatible.
    virtual bool prepareSingleFlip(MatType &, double * = nullptr) { return false; }
    virtual bool prepareSingleFlip(MatType &, int, double * = nullptr) { return false; }
    virtual DataType recomputePreparedRatio(MatType &) { return DataType(1); }
    virtual DataType finishSingleFlip(MatType &, bool, bool = true) { return DataType(1); }
    virtual int singleFlipProposalCount() const { return 0; }
    virtual DataType preparedRatio() const { return DataType(1); }
    virtual double preparedUniform() const { return 0.0; }
    virtual double preparedMinDenominator() const { return std::numeric_limits<double>::infinity(); }
