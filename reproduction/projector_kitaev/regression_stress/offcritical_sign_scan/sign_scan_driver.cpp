...
Args parse(int n,char**v){
 if(n!=19) throw std::runtime_error("usage: sign_scan_driver L theta beta_trial dt V delta mu boundary hs seed burn measurements threads raw.csv diagnostic_stride stabilization_interval guard sign_stride");
...
int main(int argc,char**argv)try{
 ...
 for(int k=0;k<a.measurements;++k){
  if(k%a.sign_stride==0){const PfaffianResult z=q.getSignRawWithStatus();++recomp;if(z.ok()&&(q.sign.real()>=0)!=(z.value.real()>=0))++corr;}
 ...
 for(int b=0;b<nb;++b)if(bn[b])brs.push_back(bs[b]/bn[b]);
 double run=std::chrono::duration<double>(std::chrono::steady_clock::now()-started).count();double gf=q.proposal_attempt_count?double(q.pre_decision_rebuild_count)/q.proposal_attempt_count:0;
 std::cout<<std::setprecision(17)<<"{\"mode\":\"static_projector_average_sign_scan\",\"L\":"<<a.L<<",\"theta\":"<<a.theta<<",\"beta_trial\":"<<a.beta<<",\"dt\":"<<a.dt<<",\"V\":"<<a.V<<",\"delta\":"<<a.delta<<",\"mu\":"<<a.mu<<",\"boundary\":"<<a.boundary<<",\"hs_scheme\":"<<a.hs<<",\"seed\":"<<a.seed<<",\"burn\":"<<a.burn<<",\"measurements\":"<<a.measurements<<",\"stabilization_interval\":"<<a.stb<<",\"sign_recompute_stride\":"<<a.sign_stride<<",\"physical_slices\":"<<ns<<",\"trial_slices\":"<<w.ntrial<<",\"center_green_norm_first\":"<<center_green_norm<<",\"sum_sign\":"<<ss<<",\"average_sign\":"<<ss/a.measurements<<",\"average_sign_err\":"<<se(brs)<<",\"acceptance\":"<<double(accepted)/attempted<<",\"runtime_seconds\":"<<run<<",\"negative_signs\":"<<neg<<",\"sign_recomputes\":"<<recomp<<",\"sign_corrections\":"<<corr<<",\"max_sign_imag\":"<<maxsi<<",\"max_observable_imag\":"<<maxoi<<",\"adaptive_guard\":"<<(a.guard?"true":"false")<<",\"guard_threshold\":"<<guard_threshold<<",\"proposal_attempt_count\":"<<q.proposal_attempt_count<<",\"min_update_denominator\":"<<q.min_update_denominator<<",\"guard_trigger_fraction\":"<<gf<<",\"adaptive_rebuild_count\":"<<q.adaptive_rebuild_count<<",\"pre_decision_rebuild_count\":"<<q.pre_decision_rebuild_count<<",\"post_accept_rebuild_count\":"<<q.post_accept_rebuild_count<<",\"full_rebuild_count\":"<<q.adaptive_rebuild_count<<",\"multiprecision_fallback\":false,\"multiprecision_fallback_count\":"<<q.multiprecision_fallback_count<<",\"diagnostic_stride\":"<<a.diag_stride<<",\"diagnostic_boundary\":0,\"diagnostic_comparisons\":"<<diag_n<<",\"diagnostic_relative_frobenius_max\":"<<maxgf<<",\"diagnostic_S_pi_abs_diff_max\":"<<maxdp<<",\"diagnostic_R_cdw_abs_diff_max\":"<<maxdr<<",\"diagnostic_sign_mismatch_count\":"<<diag_sm<<"}\n";
#ifdef PFQMC_SCALE_SAFE_UDT
 reportScaleSafeUDTStats();
#endif
 return 0;
