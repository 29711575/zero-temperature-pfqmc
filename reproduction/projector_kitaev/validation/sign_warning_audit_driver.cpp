...
namespace {
struct Args { int L,boundary,hs,seed,burn,measurements,threads,diag_stride,sign_stride; double theta,beta,dt,V,delta,mu; std::string csv; };
Args parse(int n,char**v){
 if(n!=17) throw std::runtime_error("usage: sign_warning_audit_driver L theta beta_trial dt V delta mu boundary hs seed burn measurements threads raw.csv full_diag_stride direct_sign_stride");
 Args a{std::stoi(v[1]),std::stoi(v[8]),std::stoi(v[9]),std::stoi(v[10]),std::stoi(v[11]),std::stoi(v[12]),std::stoi(v[13]),std::stoi(v[15]),std::stoi(v[16]),std::stod(v[2]),std::stod(v[3]),std::stod(v[4]),std::stod(v[5]),std::stod(v[6]),std::stod(v[7]),v[14]};
 long long nt=std::llround(a.theta/a.dt);
 if(a.L<2||a.theta<=0||a.beta<=0||a.dt<=0||a.V<0||(a.boundary!=0&&a.boundary!=1)||(a.hs!=0&&a.hs!=1)||a.burn<0||a.measurements<=0||a.threads<=0||a.diag_stride<0||a.sign_stride<=0||a.csv.empty()||std::abs(nt*a.dt-a.theta)>1e-10) throw std::runtime_error("invalid parameters");
 return a;
}
...
int main(int argc,char**argv)try{
 ...
 raw<<"measurement,sign,tracked_sign,direct_sign,direct_sign_imag,transport_direct_mismatch,sign_S_pi_numerator,sign_S_pi_dq_numerator,S_pi,S_pi_dq,R_cdw,acceptance,diag_relative_frobenius,diag_S_pi_abs_diff,diag_R_cdw_abs_diff,diag_sign_mismatch\n"<<std::setprecision(17);
 int nb=std::min(20,a.measurements);std::vector<double>bs(nb),bp(nb),bd(nb),brs,brp,brd,brr;std::vector<int>bn(nb);double ss=0,sp=0,sd=0,maxsi=0,maxdsi=0,maxoi=0;long long accepted=0,attempted=0,neg=0,recomp=0,corr=0,diag_n=0,diag_sm=0,transport_n=0,transport_mismatch_count=0;double maxgf=0,maxdp=0,maxdr=0;
 for(int k=0;k<a.measurements;++k){
  ...
  DataType direct=q.getSignRaw(); ++recomp; ++transport_n; int transport_mismatch=((sg.real()>=0)!=(direct.real()>=0)); if(transport_mismatch)++transport_mismatch_count; maxdsi=std::max(maxdsi,std::abs(direct.imag())); if(transport_mismatch){q.sign=direct.real()>=0?DataType(1,0):DataType(-1,0);++corr;}
  ...
  if(a.diag_stride>0&&k%a.diag_stride==0){MatType full;q.rebuildGreenFromFullContourAtBoundary(w.center,full);double pf=phys_spi(cfg,full),df=phys_sdq(cfg,full),rf=1-df/pf;rel=(g-full).norm()/std::max(full.norm(),std::numeric_limits<double>::min());dp=std::abs(p-pf);dr=std::abs(r-rf);sm=transport_mismatch;++diag_n;diag_sm+=sm;maxgf=std::max(maxgf,rel);maxdp=std::max(maxdp,dp);maxdr=std::max(maxdr,dr);}
  raw<<k<<','<<s<<','<<(sg.real()>=0?1:-1)<<','<<(direct.real()>=0?1:-1)<<','<<direct.imag()<<','<<transport_mismatch<<','<<s*p<<','<<s*d<<','<<p<<','<<d<<','<<r<<','<<(attempted?double(accepted)/attempted:0)<<','<<rel<<','<<dp<<','<<dr<<','<<sm<<'\n';
 ...
 std::cout<<std::setprecision(17)<<"{\"mode\":\"static_sign_warning_audit\",\"L\":"<<a.L<<",\"theta\":"<<a.theta<<",\"beta_trial\":"<<a.beta<<",\"dt\":"<<a.dt<<",\"V\":"<<a.V<<",\"delta\":"<<a.delta<<",\"mu\":"<<a.mu<<",\"boundary\":"<<a.boundary<<",\"hs_scheme\":"<<a.hs<<",\"seed\":"<<a.seed<<",\"burn\":"<<a.burn<<",\"measurements\":"<<a.measurements<<",\"direct_sign_stride\":"<<a.sign_stride<<",\"physical_slices\":"<<ns<<",\"trial_slices\":"<<w.ntrial<<",\"S_pi\":"<<p<<",\"S_pi_dq\":"<<d<<",\"R_cdw\":"<<r<<",\"average_sign\":"<<ss/a.measurements<<",\"acceptance\":"<<double(accepted)/attempted<<",\"runtime_seconds\":"<<run<<",\"sign_recomputes\":"<<recomp<<",\"sign_corrections\":"<<corr<<",\"max_sign_imag\":"<<maxsi<<",\"max_direct_sign_imag\":"<<maxdsi<<",\"max_observable_imag\":"<<maxoi<<",\"adaptive_guard\":false,\"diagnostic_stride\":"<<a.diag_stride<<",\"diagnostic_comparisons\":"<<diag_n<<",\"diagnostic_relative_frobenius_max\":"<<maxgf<<",\"diagnostic_S_pi_abs_diff_max\":"<<maxdp<<",\"diagnostic_R_cdw_abs_diff_max\":"<<maxdr<<",\"diagnostic_sign_mismatch_count\":"<<diag_sm<<",\"transport_direct_comparisons\":"<<transport_n<<",\"transport_direct_mismatch_count\":"<<transport_mismatch_count<<"}\n";
 return 0;
