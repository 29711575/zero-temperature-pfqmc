...
int main(int argc,char**argv)try{
 ...
 int nb=std::min(20,a.measurements);std::vector<double>bs(nb),bp(nb),bd(nb),brs,brp,brd,brr;std::vector<int>bn(nb);double ss=0,sp=0,sd=0,maxsi=0,maxoi=0;long long accepted=0,attempted=0,neg=0,recomp=0,corr=0,diag_n=0,diag_sm=0;double maxgf=0,maxdp=0,maxdr=0;ProjectorRawSignChecks rawChecks;
 for(int k=0;k<a.measurements;++k){
  if(k%20==0){const PfaffianResult z=q.getSignRawWithStatus();++recomp;rawChecks.record(q.sign,z);}
  ...
  if(a.diag_stride>0&&k%a.diag_stride==0){MatType full;q.rebuildGreenFromFullContourAtBoundary(w.center,full);double pf=phys_spi(cfg,full),df=phys_sdq(cfg,full),rf=1-df/pf;rel=(g-full).norm()/std::max(full.norm(),std::numeric_limits<double>::min());dp=std::abs(p-pf);dr=std::abs(r-rf);const PfaffianResult sr=q.getSignRawWithStatus();if(sr.ok()){sm=((sg.real()>=0)!=(sr.value.real()>=0));diag_sm+=sm;}++diag_n;maxgf=std::max(maxgf,rel);maxdp=std::max(maxdp,dp);maxdr=std::max(maxdr,dr);}
 ...
 std::cout<<",\"adaptive_guard\":true,\"guard_threshold\":"<<guard_threshold<<",\"proposal_attempt_count\":"<<q.proposal_attempt_count<<",\"min_update_denominator\":";
