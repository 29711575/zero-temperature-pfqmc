...
public:
    ...
    // Diagnostic-copy-only hook. It runs at the exact capture point, before
    // rightSweep updates any later operator, and must not consume RNG.
    std::function<void(int, const MatType &, DataType)> capture_diagnostic_callback;
